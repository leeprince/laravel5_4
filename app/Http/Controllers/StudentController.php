...
class StudentController extends Controller
{
    ...
    public function constructQuery()
    {
        ...
        */
    	...
    	// get():按条件返回结果集
    	/*
        $info = DB::table('student')->where('id', '>=', 1)->get();
    	dd($info);
        */
        ...
        */
    	...
    	// select:返回指定的字段
    	/*
        $info = DB::table('student')->select('id', 'name', 'sex')->where([
			['id', '>=', 1],
		])->get();
    	dd($info);
        */
    	...
    	// first():返回一行数据
    	/*
        $info = DB::table('student')->where('name', 'leeprince')->first();
    	dd($info);
        */
        ...
        */
    	...
    	/*
        $info = DB::table('student')->where(['name' => 'leeprince', 'id' => 1])->first();
    	dd($info);
        */
        ...
        */
    	...
    	// - whereBetween
    	/*
        $info = DB::table('student')->whereBetween ('age', [20,30])->get();
    	dd($info);
        */
    ...
    public function ormQuery()
    {
        ...
        */
        ...
        // 序列化 - 序列化模型 & 集合应该关闭 laravel 自动维护时间; 或者使用自动维护时间的默认时间格式
        
        // 序列化 - toArray() - 要转化模型及其加载的关联关系为数组，可以使用 toArray 方法。这个方法是递归的，所以所有属性及其关联对象属性（包括关联的关联）都会被转化为数组
        ...
        // 临时显示别隐藏的属性
        // $array = $students->makeVisible('sex')->toArray();

        // 临时隐藏被加入白名单的属性
        // $array = $students->makeHidden('name')->toArray();
        dd($array);
        

        // 序列化 - toJson() - 要转化模型为 JSON，可以使用 toJson 方法，和 toArray 一样，toJson 方法也是递归的，所有属性及其关联属性都会被转化为 JSON
    ...
    public function ctlResponse()
    {
        ...
        */
    	...
    	// - action()
    	/*
        return redirect()->action('StudentController@ctlResponseRedirect')->with([
    		'errorCode' => 503,
    		'msg' => 'redirect page - 一次性 Session'
    	]);
        */
    	...
    	// - route
    	/*
        return redirect()->route('ctl_redirect')->with([
    		'errorCode' => 503,
    		'msg' => 'redirect page - 一次性 Session'
    	]);
        */
